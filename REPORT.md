...
每個 cell 對**同一 hotset** 跑 **pread（oracle）/ async（hint）two arms**，外加每 (workload,layout) 一個 **baseline（無 prefetch）cell** 當分母。輸出欄位：`verify_cold_pct`、`verify_delivery_pct`、`fq_pread`、`fq_async`、`delivery_pct`、`preproc_us`（= `warmer_us`）、`e2e_us`（= preproc + fq）、`avg_us`、`majflt`、`minflt`，每筆並記 `ra_kb` 與當下 `loadavg/memavail`。
...
**two-arm measurement。** P0 對每個 cell 用**相同 hotset**、只切換 delivery 方式各跑一次（pread 不是新strategy，
而是對每個 strategy 的 hotset 額外做一次同步 delivery，以 isolation selection）：

| arm | delivery | 量到 | 回答的問題 |
|---|---|---|---|
...
- **彙總統計量**：報 **median**（對 cold-start 的長尾 random-I/O 比 mean 穩健），另記 **p95 / min / stdev**；improvement-% 一律對**同 batch 的 baseline（無 prefetch）arm**配對計算，避免跨 pipeline 比較。
...
**（async arm,median;完整表見 [overall_results.md](https://github.com/wongzinc/sqlite-research-project-sharing/blob/main/overall_results.md)。`open`=冷 open(db)、`deliver`=prefetch syscalls;`e2e_std`=fq+open+deliver、`e2e_warm`=fq+deliver。）**
